...
for (outcome in outcome_vars) {
  ...
  if (all(c("age_band","sex_cat") %in% cell_vars_model)) {
    extra_terms <- c(extra_terms, "age_band:sex_cat")
  }
  
  rhs_terms <- paste(c(cell_vars_model, extra_terms), collapse = " + ")
  ...
  priors <- c(
    ...
    prior(exponential(4), class = "sd", group = "lad24cd:wd24cd"), # modified (!)
