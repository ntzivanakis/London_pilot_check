...
# >>> Organisation activity only (Wave L)
geo_l <- load_wave_geo(
  filename = "l_indresp",
  vars     = c("l_orga96")  # "None of these" for organisations
)

data.table::setDT(geo_l)

# Organisation activity (0/1 at individual level)
# l_orga96 is "None of these":
#   1 = Mentioned  -> NONE of the organisations (not active)
#   0 = Not mentioned -> active in at least one organisation
geo_l[, org_activity := data.table::fifelse(
  l_orga96 %in% 0:1 & is.finite(l_orga96),
  data.table::fifelse(l_orga96 == 0, 1, 0),
  NA_real_
)]

# Keep only join key + derived indicator (person-level)
arts_sport_person <- geo_l[, .(pidp, org_activity_participation = org_activity)]
...
# Final member list: ONLY org_activity_participation
B$added_vars <- c("org_activity_participation")
...
# >>> Load Wave L geofile with the health/welfare service use item
geo_l <- load_wave_geo(
  filename = "l_indresp",
  vars     = "l_hlsv96"
)

# >>> Recode service use (0/1 → 0–1 indicator)
# l_hlsv96:
# 1 = Mentioned "None of these" → BAD → 0
# 0 = Not mentioned → used some service → GOOD → 1
geo_l[, health_service_access := fifelse(
  l_hlsv96 %in% c(0, 1) & is.finite(l_hlsv96),
  ifelse(l_hlsv96 == 0, 1, 0),   # 0 → 1 (good), 1 → 0 (bad)
  NA_real_
)]

# Keep only join key + derived indicator (person-level)
health_services_person <- geo_l[, .(pidp, health_service_access)]
health_services_person <- unique(health_services_person, by = "pidp")

# Attach PERSON-LEVEL indicator to people (by pidp)
B$dt <- merge(
  B$dt,
  health_services_person,
  by   = "pidp",
  all.x = TRUE,
  sort  = FALSE
)

# Register variable as member of block
B$added_vars <- "health_service_access"
