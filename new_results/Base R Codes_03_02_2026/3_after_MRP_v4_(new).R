...
#  5) Computes CPI (mean of domain z-scores) + optional rescale to 0-10
...
cpi_tbl <- domain_scores_z %>%
  ...
  ungroup() %>%
  mutate(
    CPI_0_10 = rescale_0_10(CPI_z)
  ) %>%
  select(wd24cd, CPI_z, CPI_0_10)
...
save_choropleth(
  ward_map_df,
  value_col = "CPI_0_10",
  title = "CPI (0–10), Greater London wards",
  subtitle = "CPI = mean of domain z-scores, rescaled to 0–10",
  out_png = file.path(OUT_SUMMARY_DIR, "map_CPI_0_10.png")
)
...
message(" - map_CPI_0_10.png, map_CPI_z.png, and map_DOM__*.png")
